...
export async function sendResetPasswordEmail(
  ...
  const from = process.env.SMTP_FROM || `"CriOverlay" <noreply@CriOverlay.com>`;
  ...
  const mailOptions = {
    from,
    to: email,
    subject: "Reset your CriOverlay Password",
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>Hello,</p>
            <p>We received a request to reset the password for your account on CriOverlay. Click the button below to set a new password. This link is valid for <strong>1 hour</strong>.</p>
          ...
          <div class="footer">
            &copy; 2026 CriOverlay. All rights reserved.
          </div>
        </div>
      </body>
      </html>
...
export async function sendAdminPaymentNotificationEmail(
  ...
  const user = process.env.SMTP_USER;
  const pass = process.env.SMTP_PASS;
  const from = process.env.SMTP_FROM || `"CriOverlay System" <noreply@CriOverlay.com>`;
    ...
    html: `
      ...
      <body>
        <div class="container">
          ...
          <div class="footer">
            System generated notification &copy; 2026 CriOverlay.
          </div>
        </div>
      </body>
      </html>
...
export async function sendAdminCustomEmail(
  ...
  const user = process.env.SMTP_USER;
  const pass = process.env.SMTP_PASS;
  const from = process.env.SMTP_FROM || `"CriOverlay Support" <support@CriOverlay.com>`;
